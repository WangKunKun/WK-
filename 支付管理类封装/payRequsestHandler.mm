...
- ( NSMutableDictionary *)sendPay_demo:(NSString *)order_id Price:(NSString *)order_price IfCharge:(BOOL)ifCharge
{
    ...
    //订单标题，展示给用户
    if (ifCharge) {
    order_name    = [NSString stringWithFormat:@"及时帮充值%@元",order_price];
    }
    else{
    order_name    = [NSString stringWithFormat:@"及时帮订单%@",order_id];
    }
    
    //订单金额,单位（分）
    NSString *orders_price   = [NSString stringWithFormat:@"%.0f",order_price.floatValue*100];//1分钱测试
//    orders_price   = @"1";
    ...
    [packageParams setObject: orders_price       forKey:@"total_fee"];       //订单金额，单位为分
